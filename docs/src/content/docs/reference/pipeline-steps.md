...
- Recognizes the optional `reviewed_paths` coverage record: the exact changed files the reviewer actually read and judged. Omitting it preserves the legacy approval behavior for callers that predate this field. When present on a clean review, it must exactly cover the trusted reviewable set computed from the current diff (the paths that survive `ignore_patterns`); empty, partial, fabricated, out-of-scope, or mixed coverage keeps the round from certifying the whole head. During carry-forward verification, omitted or invalid coverage likewise cannot clear an outstanding selected finding.
...
- Keeps an append-only outstanding finding set across Review fix rounds. Selecting a finding for a fix hands it to the fixer but does not remove it; it clears only when a later rereview lists that finding's file in `reviewed_paths`, the path belongs to the trusted reviewable set for that round, and the rereview reports neither the finding nor any other finding in that file, or when the operator explicitly approves, skips, or aborts the gate. Silence, missing or out-of-scope coverage, a rereview of another file, a shifted or reworded report in the same file, and file-less findings remain outstanding. This applies to user-authored findings too, and a selected `info` finding can therefore keep the gate parked even though an unselected informational finding is non-blocking
