...
- Requires the review output's `reviewed_paths` coverage record: the exact changed files the reviewer actually read and judged. An omitted file is unverified, not implicitly clean
...
- Keeps an append-only outstanding finding set across Review fix rounds. Selecting a finding for a fix hands it to the fixer but does not remove it; it clears only when a later rereview covers that finding's file, reports neither the finding nor any other finding in that file, or when the operator explicitly approves, skips, or aborts the gate. Silence, missing coverage, a rereview of another file, a shifted or reworded report in the same file, and file-less findings remain outstanding. This applies to user-authored findings too, and a selected `info` finding can therefore keep the gate parked even though an unselected informational finding is non-blocking
