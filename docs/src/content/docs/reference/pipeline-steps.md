...
- Requires the review output's `reviewed_paths` coverage record: the exact changed files the reviewer actually read and judged. The executor accepts a path as positive coverage only when it belongs to the trusted reviewable set computed from the current diff (the paths that survive `ignore_patterns`); omitted, fabricated, out-of-scope, or mixed coverage fails closed for that round rather than certifying a finding
